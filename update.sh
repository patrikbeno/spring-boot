...
url=$(curl -sI "${repo}/service/local/artifact/maven/redirect?r=snapshots&g=org.springframework.boot&a=spring-boot-launcher&v=${version}&e=jar" | perl -n -e '/^Location: (.*)$/ && print "$1\n"' | sed 's/\r//')
wget -q -N $url

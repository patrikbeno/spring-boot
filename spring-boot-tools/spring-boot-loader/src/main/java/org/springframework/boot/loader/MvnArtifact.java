...
public class MvnArtifact {
	...
	// note: naming convention broken intentionally for simplicity; these enums are also used for debug logging
	static enum Status { NotModified, Downloaded, Updated, Cached, Offline, NotFound, Invalid }
	...
	static public MvnArtifact parse(String uri) {
		...
		mvnuri.type = (parts.length > 3) ? parts[3] : "jar";
	...
	Archive getArchive() {
		try {
			...
			throw new MvnLauncherException("Cannot create archive for "+file, e);
		}
	}
	...
	public String asString() {
		...
		return (classifier != null)
				? String.format("%s:%s:%s:%s:%s", groupId, artifactId, resolvedVersion, type, classifier)
				: String.format("%s:%s:%s:%s", groupId, artifactId, resolvedVersion, type);
	}

...
class MvnLauncherCredentialStore {
	...
	static MvnLauncherCredentialStore instance() {
		...
		if (store == null) {
			INSTANCE = new SoftReference<MvnLauncherCredentialStore>(
					store = new MvnLauncherCredentialStore());
		}
		return store;
	}
	...
	String saveKey(String key) {
		try {
			...
			out.close();
			Log.info(
					"> Generated unique user-specific encryption key. Protect the file: %s",
					keyFile);
			return key;
		}
		catch (IOException e) {

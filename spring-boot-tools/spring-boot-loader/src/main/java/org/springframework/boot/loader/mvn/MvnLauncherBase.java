...
public abstract class MvnLauncherBase extends ExecutableArchiveLauncher {
	...
	protected List<Archive> getClassPathArchives() throws Exception {
		return getClassPathArchives(artifact.isDefined() ? artifact.asMvnArtifact()
				: null);
	}

	protected List<Archive> getClassPathArchives(MvnArtifact mvnartifact)
			throws Exception {

		// in online mode, even if debug is off, don't be totally silent: we're entering
		// possibly long running task...
		if (!MvnLauncherCfg.offline.asBoolean() && !MvnLauncherCfg.debug.asBoolean()) {
			Log.info("SpringBoot MvnLauncher: Resolving dependencies ... (%s)",
					MvnLauncherCfg.repositoryUrl.asURL(true));
		}
		...
		try {
			...
			List<MvnArtifact> artifacts = (mvnartifact != null) ? getArtifacts(connector,
					mvnartifact) : getArtifacts();
	...
	protected void launch(String[] args, String mainClass, ClassLoader classLoader)
		...
		if (!MvnLauncherCfg.execute.asBoolean()) {
			Log.info("MvnLauncher: Updated. Application will not be executed (execute=false)");
			return;
		}
	...
	private List<MvnArtifact> getArtifacts(Archive archive) {
		try {
			...
			throw new MvnLauncherException(e, "Cannot resolve artifacts for archive "
					+ archive);
		}
	}
	 ...
	 */
	protected List<MvnArtifact> getArtifacts(MvnRepositoryConnector connector,
			MvnArtifact ma) throws Exception {
		Log.debug("> Resolving main artifact: %s", ma.asString());

		File f = connector.resolve(ma);
		if (!f.exists()) {
			throw new MvnLauncherException("Cannot resolve " + ma.asString() + ": "
					+ ma.getError());
		}

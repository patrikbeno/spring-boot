 ...
 * URL-based Maven repository used as a distribution site (e.g. Sonatype Nexus)
 *
...
public abstract class MvnLauncherBase extends ExecutableArchiveLauncher {
	...
	protected String getMainClass() throws Exception {
		// mianClass is set by #getArtifacts
	...
	protected List<Archive> getClassPathArchives() throws Exception {
		...
		// in online mode, even if debug is off, don't be totally silent: we're entering possibly long running task...
		if (!MvnLauncherCfg.offline.isSet() && !MvnLauncherCfg.debug.isSet()) {
			Log.info("SpringBoot MvnLauncher: Resolving dependencies ... (%s)", MvnLauncherCfg.repositoryUrl.asURL());
		}
		...
		try {
			...
			List<MvnArtifact> artifacts = artifact.isDefined()
					? getArtifacts(connector, artifact.asMvnArtifact())
					: getArtifacts();
			...
			// report class path
			if (showClasspath.isSet() && debug.isSet()) {
	...
	protected void launch(String[] args, String mainClass, ClassLoader classLoader) throws Exception {
		if (MvnLauncherCfg.updateOnly.isSet()) {
			Log.info("MvnLauncher: Updated. Application will not be executed (updateOnly=true)");
			return;
		}
		if (MvnLauncherCfg.debug.isSet()) {

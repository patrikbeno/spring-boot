...
public class Repackager {
    ...
    /**
     * If set, {@code Spring-Boot-Dependencies} manifest attribute is populated with comma-separated list
     * of dependencies (each in form {@code groupId:artifactId:version[:packaging[:classifier]]}).
     * @param exportDependencies
     */
	...
	private Manifest buildManifest(JarFile source, Libraries libraries) throws IOException {
		...
		else if (layout instanceof Layouts.None) {
			if (startClass == null) {
				throw new IllegalStateException("Unable to find main class");
			}
			manifest.getMainAttributes().putValue(START_CLASS_ATTRIBUTE, startClass);
	...
	void populateDependencies(Manifest manifest, final Libraries libraries)
		...
		manifest.getMainAttributes().putValue(BOOT_DEPENDENCIES_ATTRIBUTE,
				deps.toString());
		for (String s : sorted) {
